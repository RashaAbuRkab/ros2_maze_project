...
class PotentialFieldPlanner(Node):
    ...
    def __init__(self):
        ...
        self.stuck_window_sec     = 5.0    # observe over 5 s window
        self.stuck_disp_threshold = 0.08   # 8 cm net displacement → stuck
        self.stuck_timer          = 0.0
        self.stuck_threshold      = 2.0    # trigger after 2 s of no progress
        ...
        # │                           + k_align×align_err)             │
        # └─────────────────────────────────────────────────────────────┘
        self.wall_target_dist   = 0.35   # desired clearance to the wall [m]
        self.k_wall             = 1.2    # gain on distance error
        self.k_align            = 2.0   # gain on alignment error
        self.wall_follow_speed  = 0.20   # forward speed while following [m/s]

        self.wall_side           = 0     # +1 = right, −1 = left, 0 = unset
        self.wall_follow_timer   = 0.0
        self.wall_follow_max_sec = 60.0  # longer timeout for serpentine maze
        self.wall_follow_min_sec = 2.0   # shorter dwell before APF re-check
    ...
    def _wall_follow_step(self, x: float, y: float,
        ...
        # ── Choose wall side on first entry ──────────────────────────────
        # The serpentine maze alternates gaps: H1 gap RIGHT, H2 gap LEFT, …
        # Right-hand rule naturally follows the right wall and finds right-side
        # gaps.  But if the right wall is far away and the left wall is close,
        # it means the robot is pressed against a left wall — follow that instead.
        if self.wall_side == 0:
            ...
            side_name = "RIGHT" if self.wall_side == 1 else "LEFT"
            self.get_logger().info(
                f'[WALL] Entering wall-follow — {side_name} wall  '
        ...
        # In a serpentine maze the goal direction is almost always blocked by
        # a wall, so "path_is_clear" is too strict.  Instead, exit when the
        # robot has travelled far enough that APF can take over meaningfully.
        # We use two relaxed criteria (either is enough after min dwell):
        #   (a) goal sector is genuinely open (original check, relaxed threshold)
        #   (b) robot has moved > 1.5 m since wall-follow started — enough to
        #       have rounded a corner and be in a new corridor
        path_is_clear   = goal_sector_dist > self.gstarp * 0.8   # relaxed from 1.1
        dwell_satisfied = self.wall_follow_timer >= self.wall_follow_min_sec

        if dwell_satisfied and path_is_clear:
            self.get_logger().info(
